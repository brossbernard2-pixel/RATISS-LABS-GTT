...
# (phase 4). Écrit proofs/PHASE4-REPLAY-<date>.md avec commande, exit,
# SHA-256 — reçu machine-indépendant (normalisation <ROOT>, règle R7).
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROOF="$ROOT/proofs/PHASE4-REPLAY-$(date +%Y%m%d-%H%M%S).md"
: > "$PROOF"
fail=0
...
step "Rejeu phase 4 (tests + vérifications world_models + juge)"
run_step "tests (pytest)" bash -c "cd '$ROOT' && python3 -m pytest -q -p no:cacheprovider 2>&1 | sed -E 's/ in [0-9]+\.[0-9]+s//'"
run_step "verifications world_models" bash -c "cd '$ROOT' && python3 scripts/wm_check.py"
