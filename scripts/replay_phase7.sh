...
# (phase 7). Écrit proofs/PHASE7-REPLAY-<date>.md avec commande, exit,
# SHA-256 — reçu machine-indépendant (normalisation <ROOT>, règle R7).
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PROOF="$ROOT/proofs/PHASE7-REPLAY-$(date +%Y%m%d-%H%M%S).md"
: > "$PROOF"
fail=0
...
step "Rejeu phase 7 (tests + vérifications examples (terrains) + juge)"
run_step "tests (pytest)" bash -c "cd '$ROOT' && python3 -m pytest -q -p no:cacheprovider 2>&1 | sed -E 's/ in [0-9]+\.[0-9]+s//'"
run_step "verifications examples (terrains)" bash -c "cd '$ROOT' && python3 scripts/examples_check.py"
